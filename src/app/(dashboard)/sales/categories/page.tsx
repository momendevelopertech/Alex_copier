...
export default function SalesCategoriesPage() {
  ...
  const fetchData = async () => {
    try {
      const [cRes, coRes] = await Promise.all([fetch("/api/sales-categories"), fetch("/api/companies")]);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setSaving(true);
    setFormError("");
    const res = await fetch(editingId ? `/api/sales-categories/${editingId}` : "/api/sales-categories", {
  ...
  const handleDelete = async (id: string) => {
    if (!(await confirmAction({ message: `${t("sales.deleteCategoryConfirm")} ${t("common.deleteConfirm")}` }))) return;
    const res = await fetch(`/api/sales-categories/${id}`, { method: "DELETE" });
    if (res.status === 401) {
      signOut({ callbackUrl: "/login" });
      return;
    }
  ...
  return (
    <div dir={dir} className="space-y-5">
      ...
      <div className="flex flex-col gap-3 rounded-2xl border border-slate-200 bg-white p-5 shadow-sm sm:flex-row sm:items-center sm:justify-between">
        <div>
          <div className="flex items-center gap-2">
            <button onClick={() => router.push("/sales")} className="inline-flex items-center justify-center rounded-lg border border-gray-300 p-2 text-gray-600 transition hover:bg-gray-50" title={t("common.back")}>
              <ArrowRight size={18} />
            </button>
            <p className="text-xs font-medium tracking-[0.2em] text-blue-600 uppercase">ERP</p>
          </div>
          <h1 className="mt-1 text-xl font-bold text-slate-900 sm:text-2xl lg:text-3xl">
            {t("sales.salesCategories")}
            <span className="ms-2 text-sm font-medium text-gray-400">({filtered.length})</span>
          </h1>
        </div>
        <button onClick={openCreate} className="inline-flex items-center justify-center gap-2 rounded-xl bg-blue-600 px-4 py-2.5 text-sm font-semibold text-white shadow-sm transition hover:bg-blue-700"><Plus size={16} />{t("sales.addCategory")}</button>
      </div>
      ...
      <FormModal open={showForm} onClose={() => { setShowForm(false); setEditingId(null); }} title={editingId ? t("sales.editCategory") : t("sales.addCategory")}>
          ...
          <div className="overflow-x-auto">
            <table className="w-full min-w-[720px]">
              <thead className="bg-gray-50">
                <tr>
                  <th className="px-4 py-3 text-start text-sm font-medium text-gray-500">{t("sales.categoryName")}</th>
